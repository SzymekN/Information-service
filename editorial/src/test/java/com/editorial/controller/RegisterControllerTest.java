...
public class RegisterControllerTest {
    ...
    void should_create_user_account_and_return_ok() throws Exception {
        //given
        String userRegistrationDtoJson = new ObjectMapper().writeValueAsString(userRegistrationDto);
        when(registerService.checkIfUserExistsByEmail(userRegistrationDto.getEmail())).thenReturn(false);
        when(registerService.checkIfUserExistsByUsername(userRegistrationDto.getUsername())).thenReturn(false);
        when(registerService.registerUserEditorialToClient(any(UserRegistrationDto.class), any(HttpServletRequest.class))).thenReturn(ResponseEntity.ok("Successfully registered user"));

        //when
        mockMvc.perform(post("/editorial/registration")
    ...
    void should_return_conflict_when_email_is_already_taken() throws Exception {
        //given
        String userRegistrationDtoJson = new ObjectMapper().writeValueAsString(userRegistrationDto);
        when(registerService.checkIfUserExistsByEmail(userRegistrationDto.getEmail())).thenReturn(true);
    ...
    void should_return_bad_request_when_caller_header_is_invalid() throws Exception {
        ...
        //when
        mockMvc.perform(post("/editorial/registration/fc")
        ...
        //then
                ...
                .andExpect(content().string("Unsuccessful registration process in editorial microservice."));
    }
    ...
    void should_create_user_account_by_client_and_return_ok() throws Exception {
        //given
        String userRegistrationDtoJson = new ObjectMapper().writeValueAsString(userRegistrationDto);
        String caller = "REGISTRATION_FROM_CLIENT";
        when(registerService.checkIfUserExistsByEmail(userRegistrationDto.getEmail())).thenReturn(false);
        when(registerService.checkIfUserExistsByUsername(userRegistrationDto.getUsername())).thenReturn(false);
    ...
    void should_create_user_account_by_client() throws Exception {
        ...
        String userRegistrationDtoJson = new ObjectMapper().writeValueAsString(userRegistrationDto);
        String caller = "REGISTRATION_FROM_CLIENT";
        when(registerService.checkIfUserExistsByEmail(anyString())).thenReturn(false);
    ...
    @Test
    void should_return_409_when_email_already_exists_in_create_user_account_by_client() throws Exception {
        // given
        String userRegistrationDtoJson = new ObjectMapper().writeValueAsString(userRegistrationDto);
        String caller = "REGISTRATION_FROM_CLIENT";
        when(registerService.checkIfUserExistsByEmail(anyString())).thenReturn(true);
    ...
    @Test
    void should_return_409_when_username_already_exists_in_create_user_account_by_client() throws Exception {
        // given
        String userRegistrationDtoJson = new ObjectMapper().writeValueAsString(userRegistrationDto);
        String caller = "REGISTRATION_FROM_CLIENT";
        when(registerService.checkIfUserExistsByEmail(anyString())).thenReturn(false);
    ...
    @Test
    void should_return_bad_request_when_caller_header_is_not_set_in_create_user_account_by_client() throws Exception {
        // given
        String userRegistrationDtoJson = new ObjectMapper().writeValueAsString(userRegistrationDto);
        String caller = "INVALID_REGISTRATION_FROM_CLIENT";
